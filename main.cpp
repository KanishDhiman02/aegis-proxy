...
asio::awaitable<void> listen(tcp::acceptor& acceptor, aegis::ConnectionPool& pool) {
    for (;;) {
        ...
        // Phase 2: always forward to backend 0. Phase 3 swaps this fixed
        // index for the hash-ring lookup - handle_client itself doesn't
        // need to change.
        asio::co_spawn(acceptor.get_executor(),
                        aegis::handle_client(std::move(socket), pool, /*backend_index=*/0),
                        asio::detached);
    }
}
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        asio::co_spawn(io_context, run(acceptor, pool), asio::detached);
